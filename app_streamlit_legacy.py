    ...
    with st.chat_message("assistant"):
        with st.spinner("Processando..."):
            try:
                ...
                # ==========================================
                # MEMÓRIA PERSISTENTE: Interceptação de Login
                # ==========================================
                was_unauthenticated = not st.session_state.agent_state.get("is_authenticated", False)
                is_now_authenticated = result.get("is_authenticated", False)

                if was_unauthenticated and is_now_authenticated:
                    cpf = result.get("current_user_cpf")
                    name = result.get("current_user_name", "")
                    
                    # 1. Muda o ID da conversa no Streamlit para o CPF da pessoa
                    st.session_state.thread_id = cpf
                    cpf_config = {"configurable": {"thread_id": cpf}}
                    
                    # 2. Tenta puxar a memória antiga do SQLite
                    cpf_state = st.session_state.graph.get_state(cpf_config)
                    
                    if cpf_state and cpf_state.values and len(cpf_state.values.get("messages", [])) > 0:
                        # HISTÓRICO ENCONTRADO! Recarregar a memória
                        st.session_state.agent_state = cpf_state.values
                        
                        # Reconstruir o chat_history visual no Streamlit
                        st.session_state.chat_history = []
                        for m in cpf_state.values.get("messages", []):
                            if isinstance(m, HumanMessage):
                                st.session_state.chat_history.append({"role": "user", "content": m.content})
                            elif isinstance(m, AIMessage) and m.content and not getattr(m, "tool_calls", None):
                                st.session_state.chat_history.append({"role": "assistant", "content": sanitize_internal_tags(m.content)})
                        
                        # Adicionar mensagem de boas-vindas do retorno
                        ...
                        # Salva a mensagem de boas-vindas no estado novo
                        st.session_state.graph.update_state(cpf_config, {"messages": [AIMessage(content=welcome_msg)]})
                        
                        st.rerun()  # Recarrega a página IMEDIATAMENTE mostrando o passado
                    else:
                        # PRIMEIRO LOGIN COM ESTE CPF
                        # Transferimos todo o histórico temporal da Triagem para o Thread definitivo (o CPF)
                        st.session_state.graph.update_state(cpf_config, result)
                        
                        # E atualizamos o estado local apenas para continuar o processamento desta primeira resposta abaixo
                        st.session_state.agent_state = result
                else:
                    # Atualizar estado local normalmente nos outros casos
